...
public class VehicleController {
    ...
    // ✅ Update KM
    @PutMapping("/{id}/km")
    public ResponseEntity<ApiResponse<VehicleResponseDTO>> updateCurrentKm(
            @PathVariable Long id,
    ...
    public ResponseEntity<ApiResponse<Page<VehicleResponseDTO>>> getPaginated(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "5") int size) {

        Page<VehicleResponseDTO> result =
                vehicleService.getAllPaginated(page, size);

        return ResponseEntity.ok(
                new ApiResponse<>(true, "Vehicle fetched successfully", result)
        );
    }
    @GetMapping("/search")
    public ResponseEntity<ApiResponse<Page<VehicleResponseDTO>>> searchVehicles(
        ...
        Page<VehicleResponseDTO> result =
                vehicleService.searchVehicles(keyword, page, size);
