...
public class OilServiceServiceImpl implements OilServiceService {
    ...
    @Override
    public OilServiceResponseDTO getLatestOilService(Long vehicleId) {

        OilService oilService = oilServiceRepository
                .findTopByVehicleIdOrderByLastServiceDateDesc(vehicleId);

        if (oilService == null) {
            throw new ResourceNotFoundException("No oil service found for vehicle");
        }
    ...
    @Override
    public Page<OilServiceResponseDTO> getByVehiclePaginated(
            Long vehicleId, int page, int size) {

        Pageable pageable = PageRequest.of(page, size);

        return oilServiceRepository.findByVehicleId(vehicleId, pageable)
                .map(this::mapToResponseDTO);
    }
